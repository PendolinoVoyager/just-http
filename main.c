...
////////////////////////////////////////////////////////
////////////////////////////////////////////////////////
// THIS BLOCK WILL GO AWAY FOR PORPER RES CONSTRUCTOR //
...
int main(int argc, char **argv) {
    int client_fd, sock_fd, received, res_size;
    ...
    char *html_index = preload_static("index.html");
    if (html_index == NULL) {
    ...
    &size);
       ...
       if (client_fd < 0) {
        ...
        if (fork() == 0) {
            ...
            g_req = parse_request(client_fd, req_buff, g_op_mode);
            if (g_req == NULL) {
                goto conn_end;
            }
            // Step 3. construct response
            attach_header(WRAP_HTML_DBG(test, title), res_buff);
